    ...
    if (!isfinite(x)) {
        ...
        *ch++ = '0';            // and we're done.  so much easier rather than passing back special cases
        ...
        if (x < 0.0) {
            *ch++ = '-';
            x = -x;
        }                       // and we're done on sign, already written. no need to pass back sign
        union {
            double d;
            uint64_t l;
        } u;
        ...
        if (l % 10 >= 5)
            l += 10;            // use the last digit to round
        l /= 10;
        if (l == 0) {
            if (*(ch - 1) == '-')
                ch--;
            *ch++ = '0';
            ...
            if (sf == 0) {
                sf = 1;
                exp++;
            }                   // e.g. l was 9999999[5-9] rounded to 10000000 which added 1 digit
...
void writeComplex(Rcomplex * col, int64_t row, char **pch)
{
    ...
    if (!ISNAN(x.i)) {
        if (x.i >= 0.0)
            *ch++ = '+';        // else writeFloat64 writes the - sign
    ...
    if (x == INT64_MIN) {
        ...
        *ch++ = '.';
        ...
        for (int i = 8; i >= 0; i--) {
            *(ch + i) = '0' + n % 10;
            n /= 10;
        }                       // always 9 digits for nanoseconds
        ch += 9;
        *ch++ = 'Z';
        ch -= squashDateTime;
    }
    ...
    if (x == NULL) {
        ...
        if (q == INT8_MIN) {    // NA means quote="auto"
            ...
            if (*tt == '\0') {
                ...
                *ch++ = '"';
                *ch++ = '"';    // test 1732.7 covers this (confirmed in gdb) so it's unknown why codecov claims no coverage
                *pch = ch;
                return;
            }
...
int GZ_compressbuff(z_stream * stream, void *dest, size_t *destLen,
...
void fwriteMain(fwriteMainArgs args)
{
    ...
    if (verbose) {
        ...
        if (args.ncol <= 50) {
            for (int j = 0; j < args.ncol; j++)
                DTPRINT(_("%d "), args.whichFun[j]);
        } else {
            for (int j = 0; j < 30; j++)
                DTPRINT(_("%d "), args.whichFun[j]);
            DTPRINT(_("... "));
            for (int j = args.ncol - 10; j < args.ncol; j++)
                DTPRINT(_("%d "), args.whichFun[j]);
        }
        DTPRINT(_
                ("\nargs.doRowNames=%d args.rowNames=%d doQuote=%d args.nrow=%"
                 PRId64 " args.ncol=%d eolLen=%d\n"), args.doRowNames,
                args.rowNames, doQuote, args.nrow, args.ncol, eolLen);
    }
    ...
    if (*args.filename == '\0') {
        ...
        f = open(args.filename,
                 O_WRONLY | O_CREAT | (args.append ? O_APPEND : O_TRUNC), 0666);
    ...
    if (verbose) {
        DTPRINT(_
                ...
                args.bom ? "true" : "false", yamlLen,
                args.colNames ? "true" : "false");
        if (f == -1)
            DTPRINT(_("\n"));
    }
    size_t headerLen = 0;
    ...
    if (headerLen) {
        ...
        if (!buff)
            STOP(_("Unable to allocate %d MiB for header: %s"),
                 headerLen / 1024 / 1024, strerror(errno));
        char *ch = buff;
        if (args.bom) {
            *ch++ = (char)0xEF;
            *ch++ = (char)0xBB;
        ...
        if (f == -1) {
            *ch = '\0';
            ...
            if (!is_ok || ret2 == -1) {
                ...
                CLOSE(f);
                STOP(_("%s: '%s'"), strerror(errwrite), args.filename);
                // # nocov end
            }
        }
    }
    ...
    if (verbose) {
        DTPRINT(_
                ("Writing %" PRId64
                 " rows in %d batches of %d rows (each buffer size %dMB, showProgress=%d, nth=%d)\n"),
                args.nrow, numBatches, rowsPerBatch, args.buffMB,
                args.showProgress, nth);
    }
    t0 = wallclock();
    ...
    if (args.is_gzip || args.is_zstd) {
        ...
        if (!zbuffPool) {
            // # nocov start
            free(buffPool);
            STOP("Unable to allocate %d MB * %d thread compressed buffers; '%d: %s'. Please read ?fwrite for nThread, buffMB and verbose options.", (size_t)zbuffSize / (1024 ^ 2), nth, errno, strerror(errno));
            // # nocov end
        }
    }
    ...
    if (nth > 1)
        verbose = false;        // printing isn't thread safe
...
#pragma omp ordered
            {
                if (failed) {
                    ...
                    if (failed_compress == 0 && my_failed_compress != 0) {
                        ...
                        if (mystream.msg != NULL)
                            strncpy(failed_msg, mystream.msg, 1000);    // copy zlib's msg for safe use after deflateEnd just in case zlib allocated the message
                    }
                    ...
                    if (me == 0 && args.showProgress
                        ...
                        if (hasPrinted || ETA >= 2) {
                            if (verbose && !hasPrinted)
                                DTPRINT("\n");
                            DTPRINT("\rWritten %.1f%% of %" PRId64
                                    " rows in %d secs using %d thread%s. "
                                    "maxBuffUsed=%d%%. ETA %d secs.      ",
                                    (100.0 * end) / args.nrow, args.nrow,
                                    (int)(now - startTime), nth,
                                    nth == 1 ? "" : "s", maxBuffUsedPC, ETA);
    ...
    if (failed) {
        ...
        if (failed_compress)
            STOP(_
                 ...
                 zlibVersion(), ZLIB_VERSION, failed_compress, failed_msg,
                 Z_FINISH, Z_BLOCK,
                 verbose ?
                 _
