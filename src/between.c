...
static Rboolean int_both_closed_open(SEXP x, R_len_t i) {
  return (INTEGER(x)[i] == NA_INTEGER ? NA_LOGICAL : ((double)INTEGER(x)[i] >= l && (double)INTEGER(x)[i] < u));
}

static Rboolean int_both_open_closed(SEXP x, R_len_t i) {
  return (INTEGER(x)[i] == NA_INTEGER ? NA_LOGICAL : ((double)INTEGER(x)[i] > l && (double)INTEGER(x)[i] <= u));
}
...
static Rboolean double_both_open_closed(SEXP x, R_len_t i) {
  return (ISNAN(REAL(x)[i]) ? NA_LOGICAL : (REAL(x)[i] > l && REAL(x)[i] <= u));
}

static Rboolean double_both_closed_open(SEXP x, R_len_t i) {
  return (ISNAN(REAL(x)[i]) ? NA_LOGICAL : (REAL(x)[i] >= l && REAL(x)[i] < u));
}

SEXP between(SEXP x, SEXP lower, SEXP upper, SEXP bounds, SEXP right) {
  ...
  if (!isLogical(right) || LOGICAL(right)[0] == NA_LOGICAL)
    error("incright must be logical TRUE/FALSE.");
  ...
  if (ALTREP(right))  { right  = PROTECT(duplicate(right));  nprotect++; }
  ...
  if (LOGICAL(bounds)[0]) {
    if (LOGICAL(right)[0]) {
      ...
      fboth  = isInteger(x) ? &int_both_closed_open  : &double_both_closed_open;
    }
  } else {
    if (LOGICAL(right)[0]) {
      flower = isInteger(x) ? &int_lower_open : &double_lower_open;
      fupper = isInteger(x) ? &int_upper_closed : &double_upper_closed;
      fboth  = isInteger(x) ? &int_both_open_closed  : &double_both_open_closed;
    } else {
      flower = isInteger(x) ? &int_lower_open : &double_lower_open;
      fupper = isInteger(x) ? &int_upper_open : &double_upper_open;
